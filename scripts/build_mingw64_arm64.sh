...
# 6) pacman -S --needed base-devel mingw-w64-clang-aarch64-clang mingw-w64-clang-aarch64-cmake zip unzip git wget
# 7) git clone https://github.com/kimwalisch/Pseudosquares-Prime-Sieve.git
...
# There is no ARM64 Windows release archive yet. Use the existing x64
# release archive as a template, then replace its x64 executable with
# the ARM64 executable built below.
TEMPLATE_VERSION=1.0
TEMPLATE_ZIP=Pseudosquares-Prime-Sieve-$TEMPLATE_VERSION-win-x64.zip
RELEASE_DIR=Pseudosquares-Prime-Sieve-$VERSION-win-arm64
TMP_DIR=$RELEASE_DIR-tmp

####################################################################

# Fetch hurchalla_util. The direct clang++ command below includes headers
...
clang++ -std=gnu++17 -static -O3 -flto -DNDEBUG -D_WIN32_WINNT=0x0A00 -Wall -Wextra -pedantic -DENABLE_MULTIARCH_ARM_SVE \
    -I../src \
    ...
    ../lib/primesieve/src/arch/arm/sve.cpp \
